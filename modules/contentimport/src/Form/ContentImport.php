...
class ContentImport extends ConfigFormBase {
  ...
  public function createNode($filedata, $contentType) {
    ...
    global $base_url;
//    $filedata = iconv('ISO-8859-1', 'UTF-8', $filedata);
    //$row[2] = iconv("windows-1256" , "utf8" , $fi);
          ...
          for ($f = 0; $f < count($fieldNames); $f++) {
            switch ($fieldTypes[$f]) {
              ...
              case 'text':
                ...
                $value_test = $data[$keyIndex[$fieldNames[$f]]];
                $value_test = iconv('windows-1256', 'UTF-8', $value_test);
                $nodeArray[$fieldNames[$f]] = [
                  'value' => $value_test,
                  'format' => 'full_html',
                ];
              ...
              case 'timestamp':
                $logVariationFields .= "Importing Content (" . $fieldNames[$f] . ") :: ";
                $dateTimeStamp = strtotime($data[$keyIndex[$fieldNames[$f]]]);
                $nodeArray[$fieldNames[$f]] = ["value" => $dateTimeStamp];
